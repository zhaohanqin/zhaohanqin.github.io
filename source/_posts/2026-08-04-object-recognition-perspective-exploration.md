...
这个仓库（孔板与桌面）把两件事放进同一套 Ultralytics YOLO26-seg 数据与训练管线，但权重、推理脚本和透视模块彼此独立。从 GitHub 克隆后，不需要本地数据和权重就能在 `results/` 里看到两套“输入 → 中间过程 → 最终输出”的完整样例。
...
系列导航：上一篇：[过柱结果数字化](/2026/08/04/cc-instrument-panel-structured-data/)；下一篇：[实验室货架视觉中枢](/2026/08/15/shelf-vision-pipeline/)
